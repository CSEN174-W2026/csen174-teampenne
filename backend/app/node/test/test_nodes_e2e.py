...
def main():
    ...
    nodes = discover_nodes() # Finding the available nodes in the VM env
    if not nodes:
    ...
    for node in nodes:
        print(f"== {node.name} ({node.host}:{node.port}) ==") # Displaying the node's name and address for clarity
